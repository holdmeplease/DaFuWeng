    ...
    if status == 0:  # 游戏未开始
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if 258 <= event.key <= 260: #载入相应数量的角色
                ...
                draw_player(screen, player, role)
                status = 2
        #fclock_tick(fps)
    elif status == 2: #掷骰子
        for i in range(6):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
        print('dice_answer', dice_answer)
        for i in range(dice_answer):
            screen.blit(map_screen, (0, 0))
            screen.blit(picture_dice[dice_answer - 1], (500, 400))
            player[cur_player].local = (player[cur_player].local + 1) % len(map_coordi)
            print('player[cur_player].local', player[cur_player].local)
        ...
        if player[cur_player].local == 5: #C楼
            ...
            if cur_player == 0:
                player[0].stop = 1
                player[0].gpa += 0.1
                draw_text(screen, (750, 340), '小E在系馆学习一天，绩点+0.1', 30, 'STXINGKA.TTF', pygame.Color('grey'))
                draw_player(screen, player, role)
        elif player[cur_player].local == 19: #主楼
            player[cur_player].money += 100
            draw_text(screen, (750, 340), '观看特将答辩受到鼓舞，绩点+0.1', 30, 'STXINGKA.TTF', pygame.Color('grey'))
            draw_player(screen, player, role)
        elif player[cur_player].local == 25: #新清
            player[cur_player].money -= 100
            draw_text(screen, (800, 340), '去新清看演出，金钱-100', 30, 'STXINGKA.TTF', pygame.Color('grey'))
            draw_player(screen, player, role)
        elif player[cur_player].local == 31: #二校门
            player[cur_player].money += 500
            player[cur_player].gpa -= 0.5
            draw_text(screen, (750, 340), '找到校外实习，金钱+500，绩点-0.5', 30, 'STXINGKA.TTF', pygame.Color('grey'))
            draw_player(screen, player, role)
            ...
            if not cur_player == 2:
                player[cur_player].stop = 1
                player[cur_player].gpa += 0.1
                draw_text(screen, (800, 340), '泡在图书馆一天，绩点+0.1', 30, 'STXINGKA.TTF', pygame.Color('grey'))
                draw_player(screen, player, role)
        cur_player = (cur_player + 1) % len(player) #下一个玩家掷骰子
        while player[cur_player].stop == 1:
            player[cur_player].stop = 0
